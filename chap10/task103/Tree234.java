...
public class Tree234 {
    ...
    // Task 10.3
    public long[] sortStart(long[] array) {
        this.sortedArray = new long[array.length];
    ...
    private void recSort(Node curNode) {

        if (curNode.isLeaf()) {
            for(int j = 0; j < curNode.getNumItems(); j++) {
                arrayAdd(curNode.getItem(j).lData);
            }
        } else {
            for(int i = 0; i <= curNode.getNumItems(); i++) {
                recSort(curNode.getChild(i));
                if (i < curNode.getNumItems()) {
                    arrayAdd(curNode.getItem(i).lData);
                }
            }
        }
    }

    private void arrayAdd(long value) {
        this.sortedArray[elemN] = value;
        elemN++;
    }
}
